...
public class MyController implements Initializable {
	...
	void modifierB(ActionEvent event) throws SQLException {
		...
		if(!identifiant.equals("")&&!bdd.exist(identifiant,id)) {
			if(bdd.connexion(identifiant, modifierMdp1T.getText(), id)) {
	...
	void prefFileMAJ() {
		...
		try {
			...
			Writer wr = new FileWriter(currentDirectory.getCanonicalPath()+"\\src\\fr\\M2L_Add\\SCPrefs");
			String data="";
	...
	void loadPrefFile() {
        ...
        {
			File currentDirectory = new File(new File(".").getAbsolutePath());
			File file = new File(currentDirectory.getCanonicalPath()+"\\src\\fr\\M2L_Add\\SCPrefs");
